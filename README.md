...
- Create two apps on Fly, one for staging and one for production:
  ...
  ```sh
  fly apps create superboard-c61e
  fly apps create superboard-c61e-staging
  ```
...
- Add a `SESSION_SECRET` to your fly app secrets, to do this you can run the following commands:
  ...
  ```sh
  fly secrets set SESSION_SECRET=$(openssl rand -hex 32) --app superboard-c61e
  fly secrets set SESSION_SECRET=$(openssl rand -hex 32) --app superboard-c61e-staging
  ```

  > **Note:** When creating the staging secret, you may get a warning from the Fly CLI that looks like this:
  >
  > ```
  > WARN app flag 'superboard-c61e-staging' does not match app name in config file 'superboard-c61e'
  > ```
  >
...
- Create a database for both your staging and production environments. Run the following:
  ...
  ```sh
  fly postgres create --name superboard-c61e-db
  fly postgres attach --app superboard-c61e superboard-c61e-db

  fly postgres create --name superboard-c61e-staging-db
  fly postgres attach --app superboard-c61e-staging superboard-c61e-staging-db
  ```
